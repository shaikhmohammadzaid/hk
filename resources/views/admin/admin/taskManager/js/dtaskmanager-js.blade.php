...
$(document).on('click','.btn-refresh',function(e){
 e.preventDefault(); 
 window.location.href = "{{ url('/admin/admin-dtaskmanager') }}";
});

  function showDetails(){
      var data = $('#frm-c').serialize();
        $.get("{{ route('showdTaskRequest') }}",data,function(data){
   ...
   $(document).on('submit','#frm-search',function(e){
    ...
    var _token = $("input[name='_token']").val();
     ...
     $.get("{{ route('searchdTaskRequest') }}",{title:title,status:status,start:start,end:end,_token:_token},function(data){
     ...
     //task assiend 
      $(document).on('submit','#frm-assign',function(e){
          ...
          alert('hii');
           ...
           $.get("{{ route('add_dtask_assign') }}",data,function(data){
      ...
      $(document).on('submit','#frm-update-assign',function(e){
          ...
          var data = $('#frm-update-assign').serialize();
           ...
           $.get("{{ route('updatedAssignTask') }}",data,function(data){
